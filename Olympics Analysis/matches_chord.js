...
function countSharedPodiums(dataset, selectedYear) {
    let medalCounts = new Map(); // {NOC} -> count
    let podiums = new Map(); // {sport-year-sex} -> Set(NOC)
    let sharedPodiums = new Map(); // {stato1-stato2} -> count
    let nationPodiumCounts = new Map(); // {NOC} -> shared podium count
    ...
    podiums.forEach(nocSet => {
        ...
        for (let i = 0; i < nocArray.length; i++) {
            for (let j = i + 1; j < nocArray.length; j++) {
                ...
                // Incrementa il contatore per ciascuna nazione coinvolta
...
function drawChordDiagram(sharedPodiums, nodes, links) {
    ...
    .domain([d3.min(links, d => d.weight) -2, d3.max(links, d => d.weight)]) // Aumenta il minimo di 2 unità
    .range([1, size ]); // Controlla lo spessore minimo e massimo
...
links.forEach(link => {
    const i = nodeIndex.get(link.source);
    const j = nodeIndex.get(link.target);
    const scaledWeight = linearScale(link.weight); // Usa la scala lineare
    matrix2[i][j] = scaledWeight;
    matrix2[j][i] = scaledWeight; // Assume grafo non diretto
});
...
function drawAdjacencyMatrix(sharedPodiums, nodes) {
    ...
    const gamma = 2; // Più è alto, più velocemente il colore diventa scuro
...
legend.append("rect")
    ...
    function highlightRibbon(source, target, value) {
        ...
        const maxValue = d3.max(Object.values(sharedPodiums)); // Ottieni il valore massimo dal dominio
