...
const simulation = d3.forceSimulation(nodes)
    .force("link", d3.forceLink(links).id(d => d.id)
        .strength(d => Math.log(1 + d.weight) / Math.log(20)) // Link più forti per pesi maggiori
        .distance(d => 20 + 200 / (1 + Math.log(1 + d.weight))) // Min 20px, max distribuito logaritmicamente
    )
    .force("charge", d3.forceManyBody()
        .strength(d => -10 - Math.log(1 + d.weight) * 10) // Maggiore repulsione per nodi con più collegamenti
    )
    .force("collision", d3.forceCollide()
        .radius(d => 10) // Evita la sovrapposizione e lascia spazio per le etichette
    )
    .force("center", d3.forceCenter(centerX, centerY)) // Posiziona i nodi centrati
    .force("x", d3.forceX().strength(0.05).x(d => width / 2))
    .force("y", d3.forceY().strength(0.05).y(d => height / 2))
    .force("linkRepulsion", forceLinkSeparation()); // Mantiene una separazione minima tra i nodi
    ...
    .data(links)
    .enter().append("line")
    .attr("stroke-width", d => Math.pow(d.weight, 1.3) * 0.2)
    ...
    .attr("r",12) // Dinamico in base al peso
    .attr("stroke", "#fff")
    ...
    simulation.on("tick", () => {
    for (let i = 0; i < 5; i++) {
        simulation.tick(); // Effettua più iterazioni per stabilizzare
    }
...
function forceLinkSeparation() {
    return function(alpha) {
        links.forEach(link => {
            const dx = link.target.x - link.source.x;
            const dy = link.target.y - link.source.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            const minDistance = 40; // Distanza minima tra nodo e link

            nodes.forEach(node => {
                if (node !== link.source && node !== link.target) {
                    const nx = node.x;
                    const ny = node.y;
                    const proj = ((nx - link.source.x) * dx + (ny - link.source.y) * dy) / (distance * distance);
                    const px = link.source.x + proj * dx;
                    const py = link.source.y + proj * dy;
                    const nodeLinkDistance = Math.sqrt((nx - px) ** 2 + (ny - py) ** 2);

                    if (nodeLinkDistance < minDistance) {
    ...
    function updateDimensions() {
        const width = Math.max(window.innerWidth/0.7 ); // Almeno 300px di larghezza
